...
class Pixel(object):
    ...
    def __init__(self, data, ctype=int):
        ...
        if not isinstance(data, tuple):
            ...
            try:
                ...
                type_ok = False
            if data.ndim != 1: type_ok = False
            if data.size < 2: type_ok = False
                
        if not type_ok:
            raise TypeError('pixel data must be a list or a tuple of single numerical values')
 
        self.data = tuple(list([ctype(data[0]), ctype(data[1])])
                          + list(data[2:]))
        
        self.x = ctype(self.data[0])
        self.y = ctype(self.data[1])
        self.xy = tuple((self.x, self.y))
    ...
    def __eq__(self, pix):
        ...
        if self.data != pix.data and self.xy == pix.xy:
            warnings.warn('pixels have same coordinates but different values')
        return self.data == pix.data
...
class Source(object):
    ...
    def __init__(self, data=None, frame_shape=None):
    ...
    def _get_pixel_neighbors(self, x, y):
        if not hasattr(self, '_nngrid'):
            X,Y = np.mgrid[-1:2,-1:2]
            self._nngrid = np.array((X.flatten(), Y.flatten()))
        nns = set()
        for inn in (np.array((x,y)).reshape((2,1)) + self._nngrid).T:
            nns.add(tuple(inn))
        nns.remove((x, y))
        return nns

    
    def add(self, pix, clip=False):
        """
    ...
    def update(self, pixs, clip=False):
        type_ok = False
        ...
        for ipix in pixs:
            self.add(ipix, clip=clip)

        
    def get_neighbors(self):
        nns = Source(frame_shape=self.shape)
        for isrc in self:
            nns.update(self._get_pixel_neighbors(isrc.x, isrc.y), clip=True)
        return nns
    ...
    def centroid(self):
        return Pixel(tuple(np.mean(self.to_array()[:,:2], axis=0)), ctype=float)
    ...
    def _run(self):
        ...
        while self._set_detection_threshold() is not None:
            ...
            while grow:
                ...
                if len(isrc) >= self.MAX_SOURCE_SIZE:
                    break
